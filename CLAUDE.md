...
- COGNIT-TECHNICAL, COGNIT-KRONOS
...
### Phase 14: Technical Whitepaper (Session 36)
Technical Whitepaper v1.0 (docs/Providence_Technical_Whitepaper_v1.0.docx):
  13 sections, ~20 pages, professional formatting with python-docx
  Sections: Executive Summary, System Architecture, Seven Subsystems (detailed),
    Core Data Model, Agent Classification & Independence, Orchestration & Pipeline,
    Portfolio Management & Execution, Risk Management Framework, Security Architecture,
    Deployment & Operations, Testing & QA, Performance Characteristics, Roadmap
  Key metrics documented: 52K+ LOC, 35 agents, 93 schemas, 1588 tests, 112 source files
  Covers all 7 subsystems with agent-level detail tables
  Documents trailing stop mechanics, error isolation, multi-layer risk management

### Phase 14 continued: Launch Plan (Session 37)
Launch Plan v1.0 (docs/Providence_Launch_Plan_v1.0.docx):
  7 sections: Current State, Launch Phases (A-F), Operational Procedures, Cost Projections, Risk Factors, Timeline, Commands
  Phase A: Pre-flight checklist (10 validation steps)
  Phase B: Shadow mode (2-4 weeks, 7 success criteria)
  Phase C: Paper trading (4-8 weeks, 8 success criteria)
  Phase D: Performance evaluation (learning batch, Sharpe > 0.5 threshold)
  Phase E: Live preparation (10 steps including Polygon upgrade, Alpaca live, governance config)
  Phase F: Live trading (gradual scale-up over 4-8 weeks)
  Cost projections: $289-449/month operating costs (Polygon + Anthropic + hosting)
  Timeline: 10-12 weeks from start to first live trade (minimum 6 weeks compressed)

### Phase 15: Pre-Flight Validation (Session 38)
Comprehensive static code audit across all 7 subsystems via parallel subagent analysis:
  Factory audit: All 35 agents verified (21 frozen, 8 adaptive, 6 perception), correct categorization ✓
  Orchestrator audit: All 4 loops verified, all metadata bridges from Phase 10 confirmed in place ✓
  Schema audit: 86 classes/enums across 10 schema files, all imports validated ✓
  API audit: 5 route modules, security middleware, deployment files reviewed ✓
Bug fixes (6 total):
  1. server.py: --reload flag now passed to uvicorn.run() (was parsed but ignored)
  2. security.py: API key validation logic fixed (was `api_key != key or not key`, now `api_key != key`)
  3. perception __init__.py: Added exports for all 6 perception agents (was empty, inconsistent with other subsystems)
  4. schemas/__init__.py: Added 8 payload schema exports (PricePayload, FilingPayload, etc.)
Pre-flight validation script (scripts/preflight.py):
  10 checks: agent imports, schema imports, factory build, orchestrator config, storage layer,
  API layer, configuration files, prompt templates, API keys, codebase metrics
  Zero external dependencies — runs with stdlib only
  Produces pass/fail summary for Launch Plan Phase A readiness

### Phase 16: Shadow Mode Infrastructure (Session 39)
SystemMode enum (providence/schemas/enums.py):
  SHADOW — signal-only, no broker interaction, simulated fills from market data
  PAPER — orders submitted to Alpaca paper trading endpoint
  LIVE — orders submitted to Alpaca live endpoint
  Progression: SHADOW → PAPER → LIVE (per Launch Plan phases B → C → E/F)
Shadow schemas (providence/schemas/shadow.py):
  ShadowSignal — frozen record of a pipeline trading signal (ticker, action, direction, confidence,
    approved/rejected, simulated entry price, simulated fill qty/notional, price_at_signal,
    realized returns at 1d/5d/20d horizons for later accuracy computation)
  ShadowRunSummary — aggregated summary per pipeline cycle (signal counts, regime state, risk mode)
  ShadowPerformanceReport — multi-run aggregate with Phase B success criteria evaluation
ShadowSignalStore (providence/services/shadow_execution.py):
  Append-only, thread-safe, JSONL persistent. Indexed by signal_id, run_id, ticker.
  Same pattern as FragmentStore/BeliefStore/RunStore. Deduplication by signal_id.
ShadowExecutionService (providence/services/shadow_execution.py):
  record_signals() — extracts positions from EXEC-VALIDATE output, creates ShadowSignals,
    simulates fills from market prices (assumes $100K shadow equity), stores in ShadowSignalStore.
  Replaces ExecutionService when system_mode=SHADOW.
Capital tier enforcement in EXEC-VALIDATE (providence/agents/execution/validate.py):
  Added SystemMode import and metadata extraction (system_mode, capital_tier).
  Gate: if capital_tier=SEED and system_mode=LIVE → force risk_mode=HALTED (blocks all orders).
  SHADOW and PAPER modes: normal validation (signals recorded, paper orders allowed).
Runner shadow mode integration (providence/orchestration/runner.py):
  ProvidenceRunner now accepts shadow_signal_store and system_mode parameters.
  run_once() injects system_mode and capital_tier into metadata for downstream agents.
  _record_shadow_signals() extracts EXEC-VALIDATE output and passes to ShadowExecutionService.
  Shadow signals captured after every main loop in SHADOW mode.
CLI --mode flag (providence/__main__.py):
  New --mode argument: SHADOW (default), PAPER, LIVE.
  _build_system() creates ShadowSignalStore when mode=SHADOW, wires into Runner.
  Data stored at {data-dir}/shadow_signals.jsonl.
Shadow report generator (scripts/shadow_report.py):
  Reads ShadowSignalStore, computes directional accuracy at 1d/5d/20d horizons,
  hypothetical returns, per-ticker breakdown, Phase B readiness criteria.
  Output: text or JSON format. Phase B criteria: >55% accuracy, >0.5 Sharpe, stability.
Tests:
  tests/unit/services/test_shadow_execution.py — 12 tests (store CRUD, persistence roundtrip,
    deduplication, indexing, service recording, price simulation, edge cases)
  tests/unit/agents/execution/test_validate_shadow.py — 5 tests (SEED+SHADOW approves,
    SEED+LIVE blocks, GROWTH+LIVE allows, PAPER+SEED allows, invalid mode fallback)

### Phase 17: Shadow Mode Completion (Session 40)
PriceBackfillService (providence/services/price_backfill.py):
  Trading day utilities: _is_trading_day(), advance_trading_days(), US_HOLIDAYS set (heuristic)
  PriceBackfillService class: fetches historical closing prices from Polygon.io at T+1/T+5/T+20
    trading days after each shadow signal, computes realized returns for accuracy evaluation.
  run(max_signals=500) — main entry point, returns summary dict (processed, updated, errors, skipped)
  _find_backfill_candidates() — approved signals with simulated_entry_price, missing returns, >2 days old
  _build_price_requests() — deduplicated (ticker, date) pairs across signals
  _fetch_prices() — async Polygon fetches with inter-request delay, returns cache dict
  _compute_returns() — creates new frozen ShadowSignal with price/return fields filled
ShadowSignalStore.update_signal() (providence/services/shadow_execution.py):
  Replaces signal by ID in all indices (_signals, _by_run, _by_ticker)
  _re_persist_all() rewrites entire JSONL file from in-memory state
  Needed because ShadowSignal is frozen Pydantic — updates create new instances
Shadow REST API (providence/api/routes/shadow.py):
  GET /api/v1/shadow/stats — store statistics (total signals, runs, approved/rejected)
  GET /api/v1/shadow/signals — list with filters (ticker, run_id, approved_only, limit)
  GET /api/v1/shadow/signals/{signal_id} — single signal detail
  GET /api/v1/shadow/summaries — run summaries (newest first)
  GET /api/v1/shadow/report — performance report with Phase B criteria evaluation
  POST /api/v1/shadow/backfill — trigger price backfill (requires polygon_client)
Shadow API schemas (providence/api/schemas.py):
  ShadowSignalResponse, ShadowRunSummaryResponse, ShadowStoreStatsResponse,
  ShadowReportResponse, BackfillTriggerResponse
API wiring:
  shadow route module registered in app.py (6th route module after config)
  AppState.shadow_signal_store field added to deps.py
  server.py build_state() creates ShadowSignalStore, passes to Runner and AppState
CLI backfill command (providence/__main__.py):
  `python -m providence backfill [--data-dir DIR] [--max-signals N]`
  Loads ShadowSignalStore from JSONL, creates PolygonClient, runs PriceBackfillService
Tests:
  tests/unit/services/test_price_backfill.py — ~21 tests (trading day utils, store update,
    backfill service with mock price client, edge cases)
  tests/unit/api/test_shadow.py — ~15 tests (stats, signals, summaries, report, backfill,
    filtering, detail, no-store 404)

### Phase 18: Operational Readiness (Session 41)
Portfolio REST API (providence/api/routes/portfolio.py):
  GET /api/v1/portfolio/snapshot — current portfolio snapshot with positions, exposure, P&L
  GET /api/v1/portfolio/positions — list all open positions
  GET /api/v1/portfolio/positions/{ticker} — single position detail
  GET /api/v1/portfolio/orders — list orders with filters (status, ticker, limit)
  GET /api/v1/portfolio/orders/{order_id} — single order detail
  GET /api/v1/portfolio/orders/stats — order counts by status
  GET /api/v1/portfolio/history — recent portfolio snapshots
  AppState expanded: portfolio_tracker, order_manager fields
  API schemas: OrderResponse, OrderStatsResponse
  server.py: creates PortfolioTracker and OrderManager with JSONL persistence
PaperTradingService (providence/services/paper_trading.py):
  execute_signals(run_result) — extracts EXEC-VALIDATE/ROUTER/GUARDIAN output, submits to Alpaca paper
  poll_order_status(max_attempts) — async fill polling with OrderManager state machine
  reconcile() — syncs PortfolioTracker with Alpaca broker state
  emergency_halt(flatten_positions) — cancel all orders, optionally close positions
  retry_failed_orders() — retry logic for failed/rejected orders
  Dual signal recording: orders submitted to Alpaca AND recorded as ShadowSignals for accuracy tracking
Runner PAPER mode integration (providence/orchestration/runner.py):
  paper_trading_service parameter, _execute_paper_signals() async method
  PAPER mode: submits orders to Alpaca paper, records shadow signals, tracks portfolio
CLI PAPER mode wiring (providence/__main__.py):
  Validates ALPACA_API_KEY and ALPACA_SECRET_KEY env vars
  Creates AlpacaClient(paper=True), OrderManager, PortfolioTracker, PaperTradingService
  Shadow signal store created for both SHADOW and PAPER modes (dual tracking)
Shadow Mode Dashboard (dashboard/shadow_dashboard.html):
  Self-contained HTML dashboard with Chart.js v4 + Luxon from CDN
  Dark trading terminal aesthetic, 6 panels:
    System Overview (mode, risk, signals, Phase B readiness badges)
    Signal Accuracy Trend (1d/5d/20d line chart with 55% threshold)
    Signal Distribution (Long/Short and Approved/Rejected doughnut charts)
    Per-Ticker Performance Table (sortable, color-coded accuracy cells)
    Recent Signals Feed (last 50 signals, auto-scroll)
    Regime Timeline (color-coded state transitions)
  Auto-refresh (30s toggleable), configurable API URL, error handling
E2E Shadow Smoke Test (scripts/e2e_shadow_test.py):
  37 checks across 7 phases, stdlib only (no pytest dependency)
  Phase 1: Import verification (all 35 agents, all schemas, all enums)
  Phase 2: Factory & storage (build_agent_registry, 4 in-memory stores)
  Phase 3: Mock data injection (MarketStateFragment fixtures for 3 tickers)
  Phase 4: Pipeline execution (frozen-only cycle with Orchestrator+Runner)
  Phase 5: Storage verification (FragmentStore, RunStore, BeliefStore, ShadowSignalStore)
  Phase 6: Shadow signal verification (signal recording, query methods)
  Phase 7: API smoke test (9 endpoint checks, skips if FastAPI unavailable)
  Exit code 0 on pass, 1 on failure. CI/CD ready.

### Phase 19: Local Testing Readiness (Session 42)
Seed demo data script (scripts/seed_demo_data.py):
  Populates all 6 stores with realistic mock data for local development/testing.
  8 tickers (AAPL, MSFT, GOOGL, AMZN, NVDA, TSLA, META, JPM), 12 pipeline runs.
  FragmentStore: PRICE and TECHNICAL fragments per ticker per run.
  BeliefStore: beliefs from 6 cognition agents (FUNDAMENTAL, TECHNICAL, MACRO, EVENT, NARRATIVE, CROSSSEC).
  RunStore: pipeline run records (main/exit/governance loops).
  ShadowSignalStore: shadow signals with simulated fills and realized returns at 1d/5d/20d.
  PortfolioTracker: 4 open positions with realistic P&L.
  OrderManager: 6 orders in various lifecycle states (FILLED, SUBMITTED, PENDING).
  Deterministic via random.seed(42). CLI: python scripts/seed_demo_data.py --data-dir data/demo
Dashboard served from API (providence/api/app.py):
  GET /dashboard — serves dashboard/shadow_dashboard.html via HTMLResponse.
  Auth-exempt path (accessible without API key).
  Dashboard URL defaults to same-origin (empty base URL) for seamless integration.
  Root endpoint updated with dashboard link.
Local launch script (scripts/launch_local.sh):
  One-command local development: seeds demo data + starts API server.
  Flags: --no-seed (reuse existing data), --seed-only (seed without starting).
  Env: PROVIDENCE_PORT (default 8000).
  Starts server with --skip-perception --skip-adaptive for fast startup.
  Prints dashboard/docs/health URLs on launch.

### Phase 20: Kronos Foundation Model Integration (Session 43)
KronosService (providence/services/kronos_service.py):
  Wraps Kronos foundation model (NeoQuasar/Kronos-base from HuggingFace Hub).
  Lazy model loading via KronosPredictor, async predict() via run_in_executor.
  CandleForecast and ForecastResult frozen dataclasses.
  Input: pandas DataFrame with OHLCV columns, output: predicted candles + direction + confidence.
  Confidence computed from candle direction agreement ratio.
  Graceful degradation: ImportError if Kronos not installed returns helpful message.
CognitKronos agent (providence/agents/cognition/kronos.py):
  FROZEN agent (zero LLM calls) — pure model inference producing BeliefObjects.
  Extracts PRICE_OHLCV fragments per ticker, builds DataFrame, runs KronosService.predict().
  _forecast_to_belief() converts ForecastResult to Belief with direction, magnitude, confidence.
  _neutral_fallback() generates low-confidence neutral belief when model unavailable.
  Invalidation conditions: 2x forecast magnitude stops + confidence threshold at 0.35.
  Added to cognition __init__.py exports (now 7 agents).
Factory wiring: CognitKronos added to _FROZEN_NO_ARGS in factory.py (36 agents total).
Orchestrator wiring: COGNIT-KRONOS added to COGNITION_AGENTS (runs in parallel with other cognition agents).
Seed data: COGNIT-KRONOS added to seed.py AGENTS list for demo data.
Forecast REST API (providence/api/routes/forecast.py):
  GET /api/v1/forecast/{ticker} — single ticker Kronos forecast from FragmentStore OHLCV data.
  GET /api/v1/forecast — batch forecasts for all watchlist tickers (capped at 10).
  Registered in app.py as 16th route module.
Portal Forecast page (providence-portal/src/app/(portal)/forecast/page.tsx):
  Grid view of all watchlist forecasts with SVG mini candle charts.
  Detail view per ticker with prediction summary, confidence bar, candle table.
  Horizon selector (5/10/20/40/60 days), direction icons, return formatting.
  Added to Sidebar and MobileNav navigation (CandlestickChart icon).
  API client: forecastApi with getTicker() and getAll() methods.
  React Query hooks: useForecasts() and useForecast() with 120s refetch.
  NEXUS-LOFI design system: cobalt/ink/paper palette, 2px radius, 140ms transitions.
