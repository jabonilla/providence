...
class Watchlist:
    ...
    def tickers(self) -> list[str]:
        ...
        return [
            entry.ticker
            for entry in sorted(self.enabled_entries, key=lambda e: (e.priority, e.ticker))
        ]
    ...
    def by_priority(self, priority: int) -> list[WatchlistEntry]:
        ...
        return [entry for entry in self.enabled_entries if entry.priority == priority]
        ...
        for ticker_data in tickers:
            if isinstance(ticker_data, str):
                ...
                entry = WatchlistEntry(
                    ticker=ticker_data, sector="", enabled=True, priority=1, tags=()
                )
            else:
    ...
    def default(cls) -> Watchlist:
        ...
        entries = [
            ...
            WatchlistEntry(
                ticker="JPM", sector="Financials", priority=1, tags=("mega_cap", "finance", "sp500")
            ),
            WatchlistEntry(
                ticker="JNJ",
                sector="Health Care",
                priority=1,
                tags=("mega_cap", "healthcare", "sp500"),
            ),
            WatchlistEntry(
                ticker="XOM", sector="Energy", priority=1, tags=("mega_cap", "energy", "sp500")
            ),
            WatchlistEntry(
                ticker="PG",
                sector="Consumer Staples",
                priority=1,
                tags=("mega_cap", "consumer", "sp500"),
            ),
