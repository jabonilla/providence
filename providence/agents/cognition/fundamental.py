"""COGNIT-FUNDAMENTAL: Fundamental Value Research Agent.

Performs financial statement analysis, valuation, and earnings quality
assessment. Reads assembled context from CONTEXT-SVC and produces
BeliefObjects via Claude Sonnet 4.
...
Thesis types:
  1. Earnings quality deterioration
  2. Balance sheet stress
  3. Margin expansion/compression
  4. Capital misallocation
  5. Hidden asset value
...
DEFAULT_PROMPT_VERSION = "cognit_fundamental_v1.0.yaml"


class CognitFundamental(BaseAgent[BeliefObject]):
    """Fundamental analysis Research Agent.

    Consumes FILING_10K, FILING_10Q, FILING_8K, PRICE_OHLCV,
    and MACRO_YIELD_CURVE MarketStateFragments.

    Produces BeliefObjects with investment theses backed by
    ...
    async def process(self, context: AgentContext) -> BeliefObject:
        ...
        Steps:
          ...
          2. ANALYZE          → Build prompt from context, send to LLM
        ...
        Returns:
            BeliefObject with investment theses.
        ...
        try:
            ...
            log.info("Starting fundamental analysis")
            ...
            log.info("Sending context to LLM")
            ...
            self._error_count_24h += 1
            raise AgentProcessingError(
                message=f"COGNIT-FUNDAMENTAL processing failed: {e}",
                agent_id=self.agent_id,
            ) from e
    ...
    def _build_user_prompt(self, context: AgentContext) -> str:
        ...
        Organizes fragments by type and formats them for the LLM.
        """
        ...
        # Identify the primary ticker (most common entity in filings)
    ...
    def _format_fragment(self, frag) -> str:
        """Format a single fragment for prompt inclusion."""
        header = (
            f"[{frag.data_type.value}] {frag.entity} "
