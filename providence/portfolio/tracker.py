...
class PortfolioTracker:
    ...
    def sync_from_broker(
        ...
        with self._lock:
            ...
            for ticker in list(self._positions.keys()):
                if (
                    ticker not in broker_tickers
                    and self._positions[ticker].side == PositionSide.FLAT
                ):
    ...
    def update_price(self, ticker: str, price: Decimal) -> None:
        ...
        with self._lock:
            ...
            unrealized_pnl_pct = float((unrealized_pnl / cost_basis * 100) if cost_basis > 0 else 0)
    ...
    def gross_exposure(self) -> float:
        ...
        with self._lock:
            if self._equity <= 0:
                return 0.0
            return sum(abs(pos.weight) for pos in self._positions.values())
